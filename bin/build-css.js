...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isFirstAttemptPackage(file) ?
    `${outputDir}/FirstAttempt.css` :
    `${outputDir}/style.css`;
...
function isFirstAttemptPackage(file) {
  return path.normalize(file).includes('packages/FirstAttempt/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@FirstAttempt/}*/src/style.scss');
