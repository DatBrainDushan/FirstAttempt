describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(FirstAttempt.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        FirstAttempt.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(FirstAttempt.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      FirstAttempt.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      ...
      const { files } = FirstAttempt.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      FirstAttempt.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(FirstAttempt.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      ...
      FirstAttempt.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ FirstAttempt }) => {
      const { files } = FirstAttempt.getState()
      FirstAttempt.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        FirstAttemptserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'FirstAttempt-core:3.2.0,FirstAttempt-transloadit:3.1.3,FirstAttempt-tus:3.1.0,FirstAttempt-dropbox:3.1.1,FirstAttempt-box:2.1.1,FirstAttempt-facebook:3.1.1,FirstAttempt-google-drive:3.1.1,FirstAttempt-instagram:3.1.1,FirstAttempt-onedrive:3.1.1,FirstAttempt-zoom:2.1.1,FirstAttempt-url:3.3.1',
    ...
    cy.get('.FirstAttempt-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.FirstAttempt-StatusBar-actions > .FirstAttempt-c-btn').click()
    cy.wait([
