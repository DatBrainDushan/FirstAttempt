...
const packageNames = await fs.readdir(new URL('../packages/@FirstAttempt', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@FirstAttempt/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@FirstAttempt/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('FirstAttempt', 2)[1]
